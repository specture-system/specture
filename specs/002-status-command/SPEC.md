...
Build a shared `internal/spec` package that consolidates all spec parsing, discovery, and querying. This replaces the ad-hoc parsing scattered across `internal/validate/parser.go` (goldmark-based parsing) and `cmd/validate.go` (file discovery). The `validate` command, `status` command, and future [`list` command](specs/004-list-command/SPEC.md) will all consume this package.
