...
This is also a prerequisite for the [frontmatter numbering spec](specs/005-frontmatter-numbering/SPEC.md) — once spec numbers move out of filenames, the list command becomes the primary way to see spec numbers.

## Design Decisions

### Building on `status` internals

The [status command spec](specs/002-status-command/SPEC.md) defines parsing logic for extracting spec metadata: name, number, status (with inference), current task, task section, and complete/incomplete task lists. The `list` command should reuse this same parsing infrastructure, applying it across all specs rather than just the current one.
...
- [x] Use `spec.ParseAll` from `internal/spec` (See [Status Command](specs/002-status-command/SPEC.md) spec) to load and parse all specs
